      ...
      { allPosts.map(post => {
        return (
          <div className="mb-3 text-base flex" key={post.slug}>
            ...
            <Link className="hover:underline hover:text-blue-600 text-blue-500 dark:text-gray-200  underline-offset-4" href={`/posts/${encodeURIComponent(post.slug)}`} title={post.title}>{post.title}</Link>
          </div>
        )
      })}
    </>
